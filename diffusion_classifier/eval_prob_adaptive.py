...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='pets',
                        choices=['pets', 'flowers', 'stl10', 'mnist', 'cifar10', 'food', 'caltech101', 'imagenet',
                                 'objectnet', 'aircraft', 'shapenet'], help='Dataset to use')
    ...
    for i in pbar:
        ...
        if os.path.exists(fname):
            ...
            if args.load_stats:
                ...
                total += 1
            continue
        image, label, unique_id = target_dataset[i]
        # Retrieve the unique ID for the image
        #unique_id = target_dataset.file_to_class[str(i)]
        print("This is the unique id", unique_id)
        with torch.no_grad():
        ...
        # Save the output, including the unique ID
        torch.save({
            'errors': pred_errors, 
            'pred': pred, 
            'label': label,
            'unique_id': unique_id  # Include the unique ID
        }, fname)
